...
#pragma comment(lib, "shlwapi.lib")	// PathCompactPathEx
...
void Read(LPSTR buffer,DWORD size)
{
	DWORD read;
	ReadConsole(GetStdHandle(STD_INPUT_HANDLE),buffer,size,&read,NULL);

	buffer[read-2] = NULL;	// Remove the new line character
}
...
void Write(LPCSTR fmt,...)
{
	...
	WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE),bigbuffer,(DWORD)strlen(bigbuffer),&written,NULL);
}

void WriteColor(int color,LPCSTR fmt,...)
{
	va_list	argptr;
	CHAR	bigbuffer[0x10000];
...
void main(int argc,char** argv)
{
	...
	if(argc < 2)
	{
		Write("No input file(s) specified\n");
	}
	else
	{
		WriteColor(WHITE|GREEN,"Started hashing %i file(s):\n",argc-1);

		char buffer[MAX_PATH];

		for(int i = 1; i < argc; i++)
		{
			PathCompactPathEx(buffer,argv[i],40,0);
			MD5File(buffer,argv[i],true,false);
		}

		WriteColor(WHITE|GREEN,"Done hashing file(s)\n");
	}
...
char* PrintMD5(uchar md5Digest[16],bool upper)
{
	char chBuffer[256];
	char chEach[10];
	int nCount;
	...
	{
		sprintf(chEach, upper ? "%02X" : "%02x", md5Digest[nCount]);
...
char* MD5String(char* szString,bool upper)
{
	...
	return PrintMD5(alg.Digest(),upper);

}

// MD5File: Performs the MD5 algorithm on a file (binar or text),
// returning the results as a char*.  Returns NULL if it fails.
char* MD5File(char* szFiletitle,char* szFilename,bool status,bool upper)
{
	FILE* file;
	md5 alg;
	int nLen;
	unsigned char chBuffer[4096];
	char nSig = '-';
	unsigned int statPrint = 0;

	if(status)
	{
		WriteColor(WHITE|GREEN,"-");
		WriteColor(GREY|BLACK,"%s %c",szFiletitle,nSig);
	}
	...
	try
	{
		...
		{
			while (nLen = fread (chBuffer, 1, 4096, file))
			{
				alg.Update(chBuffer, nLen);
				statPrint += nLen;

				if(status && statPrint > 4096<<10)
				{
					statPrint = 0;

					WriteColor(GREY|BLACK,"\b%c",nSig);
			...
			if(status)
			{
				...
				int l = 40 - strlen(szFiletitle);
				for(int i = 0; i < l; i++)
					Write(" ");

				WriteColor(GREEN,"[");
				Write("%s",PrintMD5(alg.Digest(),upper));
				WriteColor(GREEN,"]\n");
			}
